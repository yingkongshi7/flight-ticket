    ...
    if not alerts_to_send:
        if link_only:
            lines += [
                ...
                "Reason: this run used `--link-only`, so it generated manual-check links only and intentionally did not call Travelpayouts/Amadeus price APIs. `Priced results: 0` is expected in this mode.",
            ]
        elif priced_count == 0:
            lines += [
                "",
                "### No alert email was sent/prepared",
                "Reason: `Priced results` is `0` even though price APIs were enabled. Check `Source statuses` and Travelpayouts/Amadeus credentials or API availability.",
            ]
        else:
            lines += [
                "",
